...
print("run <файл> - запуск файла (В разработке...)")
print("")
...
PROGRAMS_DIR = STORAGE_DIR / "programms"
PROGRAMS_DIR = PROGRAMS_DIR.mkdir(parents=True, exist_ok=True)
...
while True:
    ...
    if cmd == "readdir":
        ...
        if not args:
            print("Использование: run <имя_файла.py> [или путь]")
            continue
            
        filename = args[0]
        filepath = Path(filename)
        
        # 🔹 Если указано просто имя файла -> ищем в PROGRAMS_DIR
        if '/' not in filename and '\\' not in filename:
            filepath = PROGRAMS_DIR / filename
        else:
            #  Если пользователь явно указал путь (например run ../test.py)
            filepath = current_dir / filename
            
        filepath = filepath.resolve() # Убираем ../ и делаем путь абсолютным
        
        if not filepath.exists():
            print(f" Файл не найден: {filepath}")
            continue
            
        # 🔐 Проверка лицензии (первые 12 символов)
        try:
            with open(filepath, 'r', encoding='utf-8') as f:
                first_line = f.read(12)
        except Exception as e:
            print(f"{TRANSLATIONS_DB['file_error'][lang]}: {e}")
            continue
            
        VALID_KEY = "CleykaCore12"  # <- ТВОЙ КЛЮЧ
        if programm_key not in :
            print("Program pirated or unauthorized, it will not be activated.")
            continue
            
        # ✅ Подтверждение
        prog_name = filepath.stem.replace('_', ' ').title()
        confirm = input(f"Вы уверены что хотите запустить {prog_name}? ({filepath.name}) [y/N] ").strip()
        if confirm.lower() == 'n':
            print("Запуск отменён.")
            continue
            
        # 🚀 Запуск
        print(f"⚙️  Запуск {prog_name}...")
        try:
            import importlib.util
            spec = importlib.util.spec_from_file_location(filepath.name, filepath)
