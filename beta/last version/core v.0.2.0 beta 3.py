...
SECURITY_LEVEL = 0  # 0 = Предупреждение, 1 = Ошибка
    ...
    if cmd == "readdir":
        ...
        if is_authorized:
            ...
            if isinstance(meta, dict):
                prog_name = meta.get("name", prog_name)
        ...
        if not is_authorized:
            if SECURITY_LEVEL == 1:
                ...
                print("Программа не найдена в базе. Запуск на свой страх и риск.")

        print(f"\nИмя файла: {target_path.name}")
        print(f"Имя программы: {prog_name}")
        print(f"Версия: {prog_version}")
        print(f"Автор: {prog_author}")
        print(f"Статус: {'Авторизовано' if is_authorized else 'Неавторизовано'}\n")
        ...
        print(f"Запуск {prog_name} v{prog_version}...")
        try:
